...
class ActSep():
    ...
    def cf(self, word):
        if self.com_dict.get(word, None) != None:
            ...
            return 'Word "{}" is not in the collection!'.format(word)
    ...
    def tf(self, word, docID):
        if self.com_dict.get(word, None) != None:
            if docID in self.com_dict[word]:
                ...
                return 'Word "{}" is not in the doc # {}'.format(word, docID)
        else:
            return 'Word "{}" is not in the collection!'.format(word)
    ...
    def intersection(self, words):
        terms = words.split()
        result = set(self.com_dict[terms.pop(0)].keys())
        while terms:
            result = result.intersection(self.com_dict[terms.pop(0)].keys())
        return result

    def osm(self, words):
        terms = words.split()
        docIDs = self.intersection(words)
        if not docIDs:
            return 'Document with such terms wasn\'t found'
        else:
            holder = []
            for docid in docIDs:
                score = 0
                for term in terms:
                    score += self.tf_idf(term, docid)
                holder.append((docid, score))
        return sorted(holder, key=lambda x: x[1], reverse=True)
