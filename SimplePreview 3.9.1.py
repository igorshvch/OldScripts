...
class DocumentIndexing(TextProcessor):
    ...
    def index_creation(self):
        with lock:
            ...
            self.index_store_docs = self.index_store
    ...
    def text_indexing_docs(self):
        with lock:
            ...
            for i in range(length):
                holder[i] = [word.strip('.,-:;!?()\'\"')
                             for word in holder[i]]            
            counter = 0
            for key in self.keys:
                self.index_store_docs[key] = [i for i in range(len(holder))
                                              if key in holder[i]]
...
class WidgetGenerator(SearchEngine, ComparisonEngin, tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        left_b_buttons = [['Clean the text', lambda: thread.start_new_thread(self.wg_text_clean, ())], #threaded
                          ...
                          ['Find common\nwords', self.wg_common_words_finding],
        ...
        self.commands_for_writing = {
            ...
            'index_store_docs' : self.index_store_docs,
            'index_store_items' : self.index_store.items(),
            'index_store_docs_items' : self.index_store_docs.items(),
        ...
        self.commands_for_console = {
            ...
            'print_tk_var' : None,
            'size_of_index_store' : sys.getsizeof(self.index_store),
            'size_of_store' : sys.getsizeof(self.store),
            'size_of_par_holder' : sys.getsizeof(self.paragraph_holder)
            }
    ...
    def make_buttons(self, parent=None, button_text_command_dict=None,
                     height=1, width=13, side='top', anchor='w'):
    ...
    def make_widgets(self):
        ...
        leftBar_radiobuttons_container = tk.Frame(leftBar)
        ...
        leftBar_buttons_container.pack(side='top', fill='y')
        leftBar_radiobuttons_container.pack(side='bottom', fill='y', pady=3)
        ...
        self.make_radiobuttons(parent=leftBar_radiobuttons_container,
    ...
    def write_to_file(self):
        ...
        if text not in ['', ' ', '\n', ' \n', '\n ', ' \n ']:
            ...
            w.writer(self.commands_for_writing[write_inf[0]], write_inf[1])
        else:
