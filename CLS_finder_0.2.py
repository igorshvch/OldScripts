...
class GUIContainer(tk.Tk):
    ...
    def LCS_main(self):
        ...
        if len(list1) == len(list2) == lenLCS :
            self.lists_similarity.set('Фрагменты не содержат различий')
        else:
            resultlist1 = list1[:]
            for i in range(len(resultlist1)):
                if i not in num_box_list1:
        ...
        self.lenlist1.set('The length of the 1 list is: %d' % len(resultlist1))
        self.lenlist2.set('The length of the 2 list is: %d' % len(resultlist2))
        self.lenLCS.set('The length of LCS is: %d' % lenLCS)
...
class MainPage(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label_top = ttk.Label(text='Поиск общих последовательностей', font = FONTSTYLE)
        label_top.grid(row=0, column=0, columnspan=2)
        
        entry_str1 = ttk.Entry(textvariable=controller.string_1)
        entry_str1.grid(row=1, column=0, sticky='w')

        button1_process = ttk.Button(text='Process', command=lambda: controller.LCS_main())
        button1_process.grid(row=1, column=1, sticky='e')

        entry_str2 = ttk.Entry(textvariable=controller.string_2)
        entry_str2.grid(row=2, column=0, sticky='w')
        
        button2_clean = ttk.Button(text='Clean', command=lambda: controller.clean())
        button2_clean.grid(row=2, column=1, sticky='e')

        label_mid_inform_1 = ttk.Label(textvariable=controller.lists_similarity)
        label_mid_inform_1.grid(row=3, column=0, columnspan=2, sticky='w')
        label_mid_inform_2 = ttk.Label(textvariable=controller.lenlist1)
        label_mid_inform_2.grid(row=4, column=0, columnspan=2, sticky='w')
        label_mid_inform_3 = ttk.Label(textvariable=controller.lenlist2)
        label_mid_inform_3.grid(row=5, column=0, columnspan=2, sticky='w')
        label_mid_inform_4 = ttk.Label(textvariable=controller.lenLCS)
        label_mid_inform_4.grid(row=6, column=0, columnspan=2, sticky='w')

        text_str1 = CustomTextWidget(wrap='word')
        text_str1.grid(row=7, column=0, columnspan=2)
        text_str1.insert('1.0', controller.string_1_p.get())
        text_str1.tag_configure("yellow", background="yellow")
        text_str1.highlight_pattern('\*\w+\*', 'yellow')

        text_str2 = CustomTextWidget(wrap='word')
        text_str2.grid(row=8, column=0, columnspan=2)
        text_str2.insert('1.0', controller.string_2_p.get())
        text_str2.tag_configure("yellow", background="yellow")
        text_str2.highlight_pattern('\*\w+\*', 'yellow')
