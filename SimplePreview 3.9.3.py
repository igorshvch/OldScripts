...
class TextProcessor():
    ...
    def acts_separation(self):
        with lock:
            self.store = re.split(self.pattern_sep, string=self.cleaned)#[:100]#####
...
class WidgetGenerator(SearchEngine, ComparisonEngin, tk.Tk):
    ...
    def make_widgets(self):
        ...
        leftBar_example = tk.Frame(leftBar)#
        ...
        pads1_dict = self.make_notebook(parent=center_maintext_container,
                                        pad_names=['Main retrieval window', 'Not ready'])
        self.txt = self.make_text_with_scroll(parent=pads1_dict[0])#center_maintext_container)
        ...
        self.make_buttons(parent=leftBar_buttons_container,
                          ...
                          width=13)
        pads1_dict = self.make_notebook(parent=center_top_cons_buttons_container,
                                       pad_names = ['Main console menu', 'Not ready'])
        self.make_buttons(parent=pads1_dict[0], #center_top_cons_buttons_container,
                          button_text_command_dict=self.console_buttons,
                          height=2,
                          width=10, side='left')

        ######## Making Notebook
        pads2_dict = self.make_notebook(parent=leftBar_notebook_container,
                                       pad_names = ['Index\noptions', 'Not\nready'])
        ...
        self.make_radiobuttons(parent=pads2_dict[0], #nbframe1,
